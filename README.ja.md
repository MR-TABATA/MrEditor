...
配布用ディスクイメージ（`.build/MrEditor-1.5.dmg`）の作成:
...
- **1.5 — テキスト変換ツールボックス（書式メニュー）: ケース変換・URL/Base64/HTML エンコード＆デコード・行のソート/重複削除/逆順/連番・外部コマンドに通す（⌥⌘R）で選択を任意のシェルに流す。両ペインで動くので 10 GB ファイル内でも効く** ✅（このリリース）
