...
配布用ディスクイメージ（`.build/MrEditor-1.10.dmg`）の作成:
...
- **1.10 — 答えが書かれる端から流れてくる（待ってから一気に出るのをやめた）。モデルは打ち込みもできるドロップダウンになり、接続テストで鍵・モデル ID・エンドポイントを事前に確かめられる（一覧に無いモデルは直接入力でき、テストに通れば次から一覧に出る）。失敗は利用者の言語で何が起きたかを言い、プロバイダの原文はその下に残す** ✅（このリリース）
