...
配布用ディスクイメージ（`.build/MrEditor-1.10.3.dmg`）の作成:
...
- **1.10.3 — 日本の全法人 581 万 6535 行・1.18GiB の CSV を開いて初めて見えた 2 件。日本語環境なのにメニューバーが「ファイル・Edit・書式・View」と混ざっていた。この 2 つだけ翻訳テーブルを通っていなかった。もう 1 つは構造化表示の列幅で、先頭 1000 行だけで測っていたため後半で桁が伸びた列が切れていた —— そのファイルの末尾付近で連番の列が `581…` と省略された。列幅は両端から測るようにした** ✅（このリリース）
