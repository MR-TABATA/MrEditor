...
配布用ディスクイメージ（`.build/MrEditor-1.10.1.dmg`）の作成:
...
- **1.10.1 — 普通のファイルで ⌘F が効くようになった。検索と置換は読み取り専用の巨大ファイル用にしか作っておらず、8MB 未満 —— つまり実際に編集するファイル —— では ⌘F がビープするだけで、メニューも灰色だった。同じペインで行へ移動（⌘L）も死んでいて、アンドゥは窓の中の全ドキュメントで共有されていた** ✅（このリリース）
