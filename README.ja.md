...
- **行操作**: ソート（昇順/降順）・重複行削除（初出順を保持）・逆順・連番。
...
配布用ディスクイメージ（`.build/MrEditor-1.7.dmg`）の作成:
...
- **1.7 — 検索・置換の正規表現で先読み/後読み対応、ログ中の ANSI カラー表示（閲覧時にエスケープを着色し本文からは除去）、ウィンドウ全体の背景透明度（iTerm 風の半透明）** ✅（このリリース）
