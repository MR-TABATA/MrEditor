...
*等倍（早送りなし）。ただし**2 つの場面をつないで**います —— 10.00GB を開くところと、背後の索引が完了したあとに末尾行へジャンプするところ。ステータスバーに注目してください。索引の構築中（約 20 秒）は行数が概算で、完了すると正確な **86,420,337 行**に収束します。その間も表示は一度も止まらず、読む・探す・編集するができます。[30 秒の全編（無編集）はこちら。](docs/media/mreditor-10gb.mp4)*
