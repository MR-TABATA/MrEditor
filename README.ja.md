...
- 入口は 3 つ: **2 つのファイルを選ぶ**（⇧⌘D）・**開いているドキュメント同士**（未保存の本文もそのまま）・**クリップボードと比較**。
...
- **マージ（v1.2 で追加）** — 差分の横の**矢印をクリック**すると、その差分だけ右を取り込む（もう一度で取り消し。⌥→ / ⌥← でも可）。
  **左が土台**で、採用した差分だけが入る。**表示 ▸ 比較（diff）▸ マージ結果を別名で保存…** で書き出す。
  **元の 2 つのファイルには触らない。** 何も採用しなければ、左がそのまま出る。
...
配布用ディスクイメージ（`.build/MrEditor-1.2.dmg`）の作成:
...
- **1.2 — マージ: 差分の横の矢印をクリックして右を取り込み、結果を別名で保存する。元の 2 ファイルには触らない** ✅（このリリース）
